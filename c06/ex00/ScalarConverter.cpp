...
void ScalarConverter::convert(std::string str){
	...
	{
		char char_ptr = static_cast<char>(str[0]);
		ptrd = static_cast<double>(char_ptr);
	}
	std::string end_str = "";
	...
	(std::isprint(ptrd) && !out_of_range) ? std::cout <<"char = \'" << ptrc <<"\'"<< std::endl : (out_of_range || ptrd < 0 || ptrd > 127) ? std::cout <<"char = " << "impossible" << std::endl : std::cout <<"char = " << "Non displayable" << std::endl;
